...
const winningSel = document.querySelector("#sel");
...
p1Button.addEventListener('click', function(){
    if (!isGameOver){
        ...
        if (p1Score===winningScore){    
            isGameOver=true;
            p1Display.classList.add('winner');///if pi wins
...
p2Button.addEventListener('click', function(){
    if (!isGameOver){
        ...
        if (p2Score===winningScore){    
            isGameOver=true;
            p2Display.classList.add('winner');//if p2 wins
...
winningSel.addEventListener('change',function(){
